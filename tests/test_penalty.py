...
class Test(unittest.TestCase):
  def test_CV(self):
    pred = inverse.GCV(penalty_range,f_nonunique,data,M,sigma=sigma,system_args=(x,),regularization=reg)
    best_pred_GCV = penalty_range[np.argmin(pred)]

    pred = inverse.CV(penalty_range,f_nonunique,data,M,sigma=sigma,system_args=(x,),regularization=reg)
    best_pred_CV = penalty_range[np.argmin(pred)]

    pred = inverse.KFCV(N,penalty_range,f_nonunique,data,M,sigma=sigma,system_args=(x,),regularization=reg)
